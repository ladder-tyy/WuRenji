...
float pi = 3.1415926;
float AcceRatio = 16.384f;                  //加速度计比例系数
...
void imuInit(void){
    ...
    {
        ax0 += accelEvent.acceleration.x;
        ay0 += accelEvent.acceleration.y;
        az0 += accelEvent.acceleration.z;
        gx0 += gyroEvent.acceleration.x;
        gy0 += gyroEvent.acceleration.y;
        gz0 += gyroEvent.acceleration.z;
    }
...
void imuDeal(void *pvParameters){
    while(1){
        ...
        ax = accelEvent.gyro.x;
        ay = accelEvent.gyro.y;
        az = accelEvent.gyro.z;

        float accx = ax / AcceRatio;              //x轴加速度
        float accy = ay / AcceRatio;              //y轴加速度
        float accz = az / AcceRatio;              //z轴加速度
    
        agx += atan(accy / accz) * (-cycle) / pi * dt;    //y轴对于z轴的夹角
        agy += atan(accx / accz) * cycle / pi * dt;       //x轴对于z轴的夹角
        agz += atan(-accx / accy) * cycle / pi * dt;       //z轴对于y轴的夹角
        ...
        if(agx > cycle){agx -= cycle;}
        else if(agx < -cycle){agx += cycle;}

        if(agy > cycle){agy -= cycle;}
        else if(agy < -cycle){agy += cycle;}

        if(agz > cycle){agz -= cycle;}
        else if(agz < -cycle){agz += cycle;}
